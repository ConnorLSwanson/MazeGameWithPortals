	...
	, m_pLevel(nullptr)
{
	...
	m_pPortManager = new PortalManager();
}
...
void GameplayState::HandleCollision(int newPlayerX, int newPlayerY)
{
	...
	if (collidedActor != nullptr && collidedActor->IsActive())
	{
		...
		{
			PortalManager* collidedPortal = dynamic_cast<PortalManager*>(collidedActor);
			assert(collidedPortal);
			Point p = m_pPortManager->Teleport(newPlayerX, newPlayerY);
			newPlayerX = p.x;
			newPlayerY = p.y;
			m_player.SetPosition(newPlayerX, newPlayerY);
			break;
